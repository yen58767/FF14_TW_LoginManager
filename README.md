...
- 該工具僅供個人學術用途使用
- 本工具可能涉及帳號密碼、自動化操作等敏感功能，使用者需自行承擔使用風險。
- 若因使用本工具造成帳號異常、封鎖、資料遺失、安全問題或任何直接/間接損失，開發者不承擔任何責任。
- 本工具不會主動上傳使用者帳密，但使用者應自行確保執行環境安全、設定檔妥善保存。
- 下載本工具即視為使用者同意自行承擔所有風險，並放棄向開發者尋求賠償或法律責任。
